            ...
            <Container className={data ? '' : 'no-data'}>
                <Row>
                    ...
                    {data?.map((item, key) => <Col xs={12} sm={6} md={4} className={`inner-content-each ${item.cta_show ? '' : 'remove-bottom-padding'}`}
                        ...
                        {item.image && <img src={item.image} alt="content image" className="inner-content-image"/>}
                        <h3 className="inner-content-heading">{item.title}</h3>
                        {item.imageBig && <img src={item.imageBig} alt="content image" className="inner-content-image-big"/>}
                        <p className="body-copy" dangerouslySetInnerHTML={{ __html: item.description }}/>
